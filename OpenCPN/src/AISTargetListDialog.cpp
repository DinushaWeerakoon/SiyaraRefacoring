...
void AISTargetListDialog::CreateControls()
{
    ...
    /*
     Added by Randika Ariyarathna
     */
    m_pButtonAnchorWatch = new wxButton(this, wxID_ANY, _("Set Anchor Watch"), wxDefaultPosition, wxDefaultSize, wxBU_AUTODRAW);
        ...
        for( it = ( *current_targets ).begin(); it != ( *current_targets ).end(); ++it, ++index ) {
            ...
            if( NULL != pAISTarget ) {
                ...
                if(b_add){
					...
					fp = fopen("UpdateList.txt", "a+");
					fprintf(fp, "%s", "MMSI ");
					fprintf(fp, "%d", pAISTarget->MMSI);
					fprintf(fp, "%s", " Lat ");
					fprintf(fp, "%f", pAISTarget->Lat);
					fprintf(fp, "%s", " Lon ");
					fprintf(fp, "%f", pAISTarget->Lon);
					fprintf(fp, "%s", "\n");
