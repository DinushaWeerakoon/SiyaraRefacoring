 ...
 */
wxDatePickerCtrl *calender;
wxTextCtrl *historyFrom;
...
//    SetDefaultConnectionParams();
    ...
    wxStaticBox* HistoryBox = new wxStaticBox(AISFilterPanel, wxID_ANY, _("View History"));
    wxStaticBoxSizer* HistoryBoxSizer = new wxStaticBoxSizer(HistoryBox, wxVERTICAL);
    AISFilterPanelSizer->Add(HistoryBoxSizer, 0, wxEXPAND | wxALL, border_size);
    ...
    wxStaticText *historyOn = new wxStaticText(AISFilterPanel, wxID_ANY, _("Date : "));
    HisoryGrid->Add(historyOn, 1, wxALIGN_CENTER_HORIZONTAL, group_item_spacing);
    
    calender = new wxDatePickerCtrl(AISFilterPanel, wxID_ANY, wxDefaultDateTime);
    HisoryGrid->Add(calender, 1, wxALIGN_CENTER_HORIZONTAL, group_item_spacing);
    
    //-------Spacer-------
    HisoryGrid->Add(5, 8, 0, wxALL, 5); HisoryGrid->Add(5, 8, 0, wxALL, 5);
    
    wxStaticText *From = new wxStaticText(AISFilterPanel, wxID_ANY, _("Time : "));
    HisoryGrid->Add(From, 1, wxALIGN_CENTER_HORIZONTAL, group_item_spacing);
    
    historyFrom = new wxTextCtrl(AISFilterPanel, wxID_ANY, _("10:00:00"));
    HisoryGrid->Add(historyFrom, 1, wxALIGN_CENTER_HORIZONTAL, group_item_spacing);
    
    wxStaticText *hint = new wxStaticText(AISFilterPanel, wxID_ANY, _("Format of time should be 10:00:00"));
    HisoryGrid->Add(hint, 1, wxALIGN_CENTER_HORIZONTAL, group_item_spacing);
    
    wxButton* historyButton = new wxButton(AISFilterPanel, ID_HISTORY_QUERY, _("View "));
...
{
    wxString timeS = historyFrom->GetValue();
    if (timeS.Len() != 8 || timeS.at(2) != ':' || timeS.at(5) != ':')
    {
        wxString msg(_("Format error in time."));
        OCPNMessageDialog md(NULL, msg, _("Siyara Message"), wxICON_ERROR | wxCENTRE);
        md.ShowModal();
    }
    else{
        ...
        wxString minLb = dateTime.Subtract(*diffTimeL).FormatISOTime();
        //wxLogMessage(minLb);
        
        const wxTimeSpan *diffTimeU = new wxTimeSpan(0, 30, 0, 0);
        wxString minUb = dateTime.Add(*diffTimeU).FormatISOTime();
        //wxLogMessage(minUb);
        
        std::string timeLb = std::string(minLb.mb_str());
        std::string timeUb = std::string(minUb.mb_str());
        
        wxDateTime date = calender->GetValue();
