...
//                sart_sub_type = _T("SART");
    ...
    if( b_positionOnceValid ) {
        ...
        int now_seconds = nowt.GetSecond();
        int now_minutes = nowt.GetMinute();
        int now_hour = nowt.GetHour();
        
        wxString str_seconds, str_minutes, str_hours;
        str_seconds << now_seconds;
        str_minutes << now_minutes;
        str_hours << now_hour;
        
        wxString now_string = str_hours;
        now_string.append(_T(":"));
        now_string.append(str_minutes);
        now_string.append(_T(":"));
        now_string.append(str_seconds);
        
        char dbTime[9], nowTime[9];
        strncpy(dbTime, (const char*)db_reported_time.mb_str(wxConvUTF8), 8);
        strncpy(nowTime, (const char*)now_string.mb_str(wxConvUTF8), 8);
        
        int h1, m1, s1;
        int h2, m2, s2;
        
        sscanf(nowTime, "%d:%d:%d", &h1, &m1, &s1);
        sscanf(dbTime, "%d:%d:%d", &h2, &m2, &s2);
        
        int difference = (h1 - h2) * 3600 + (m1 - m2) * 60 + (s1 - s2);
        //wxLogMessage(_T("Age of = %d"), different);
        
        //line.Printf(_("Target updated before:       %d Sec.\n\n"), different);
        
        //int target_age = now.GetTicks() - PositionReportTicks;
        int target_age = difference;
        //-wrc-----------------------
