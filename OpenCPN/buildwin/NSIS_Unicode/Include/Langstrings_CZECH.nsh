#LangString Installer ${LANG_ENGLISH} "installer"
LangString Installer ${LANG_CZECH} "instalátor"

#LangString Uninstaller ${LANG_ENGLISH} "uninstaller"
LangString Uninstaller ${LANG_CZECH} "deinstalátor"

#LangString Installer_Unsupported_Windows_Version ${LANG_ENGLISH} "Your Windows version is not supported (it's too old)!$\r$\nYou need at least Windows NT/2000."
LangString Installer_Unsupported_Windows_Version ${LANG_CZECH} "Vaše verze Windows není podporována (je moc stará)!$\r$\nPotřebujete alespoň Windows NT/2000."

#LangString Installer_Running ${LANG_ENGLISH} "The installer is already running."
LangString Installer_Running ${LANG_CZECH} "Instalátor již je spuštěn."

#LangString Uninstaller_Running ${LANG_ENGLISH} "The uninstaller is already running."
LangString Uninstaller_Running ${LANG_CZECH} "Deinstalátor již je spuštěn."

#LangString LangDialog_Title ${LANG_ENGLISH} "OpenCPN 4.1.718 Installation"
LangString LangDialog_Title ${LANG_CZECH} "Instalace OpenCPN 4.1.718"

#LangString LangDialog_Text ${LANG_ENGLISH} "Please select a language:"
LangString LangDialog_Text ${LANG_CZECH} "Prosím vyberte jazyk:"

#LangString text_please_wait_1 ${LANG_ENGLISH} "... please wait!"
LangString text_please_wait_1 ${LANG_CZECH} "... prosím čekejte!"

#LangString Scanning_HDD ${LANG_ENGLISH} "Scanning HDD ..."
LangString Scanning_HDD ${LANG_CZECH} "Skenuji disk ..."

#LangString OCPN_Scanning_HDD ${LANG_ENGLISH} "OpenCPN: Scan HDD ..., please wait!"
LangString OCPN_Scanning_HDD ${LANG_CZECH} "OpenCPN: Sken disku ..., prosím čekejte!"

#LangString Search_old_installations ${LANG_ENGLISH} "Searching for old installations"
LangString Search_old_installations ${LANG_CZECH} "Hledání starších instalací"

#LangString Install_Location_Default ${LANG_ENGLISH} "Install Locations by default paths (may not find all)"
LangString Install_Location_Default ${LANG_CZECH} "Místa instalace podle výchozích cest (nemusí najít vše)"

#LangString Install_Location_HDD_Scan ${LANG_ENGLISH} "Install Location by full HDD scan (most reliable)"
LangString Install_Location_HDD_Scan ${LANG_CZECH} "Místo instalace pomocí skenu celého disku (nejspolehlivější)"

#LangString Label_CleanUp_Text ${LANG_ENGLISH} "Click on a list-item to select/ unselect it or Select ALL (recommended) and then Uninstall selected."
LangString Label_CleanUp_Text ${LANG_CZECH} "Klikněte na položku seznamu pro její označení/ zrušení nebo Označit VŠE (doporučeno) a pak odinstalujte vybrané."

#LangString BT_Uninstall_Selected_CleanUp ${LANG_ENGLISH} "Uninstall Selected"
LangString BT_Uninstall_Selected_CleanUp ${LANG_CZECH} "Odinstalovat vybrané"

#LangString BT_SelectAll_CleanUp ${LANG_ENGLISH} "Select ALL"
LangString BT_SelectAll_CleanUp ${LANG_CZECH} "Označit VŠE"

#LangString BT_UnselectAll_CleanUp ${LANG_ENGLISH} "Unselect ALL"
LangString BT_UnselectAll_CleanUp ${LANG_CZECH} "Zrušit VŠE"

#LangString LB_CleanUp ${LANG_ENGLISH} "Listbox Text"
LangString LB_CleanUp ${LANG_CZECH} "Text listboxu"

#LangString BT_FullScan_CleanUp ${LANG_ENGLISH} "Scan HDD ..."
LangString BT_FullScan_CleanUp ${LANG_CZECH} "Sken disku ..."

#LangString LAB_FullScan_CleanUp ${LANG_ENGLISH} "Scan the HDD for the most reliable detection.$\r$\nATTENTION: may last several minutes!"
LangString LAB_FullScan_CleanUp ${LANG_CZECH} "Sken disku pro nejspolehlivější detekci.$\r$\nPOZOR: může trvat několik minut!"

#LangString Page_CleanUp_Title ${LANG_ENGLISH} "Uninstall older Version"
LangString Page_CleanUp_Title ${LANG_CZECH} "Odinstalovat starší verzi"

#LangString Page_CleanUp_Subtitle ${LANG_ENGLISH} "Incompatible version detected"
LangString Page_CleanUp_Subtitle ${LANG_CZECH} "Nalezena nekompatibilní verze"

#LangString CP_HEADER_TEXT ${LANG_ENGLISH} "Installation Settings"
LangString CP_HEADER_TEXT ${LANG_CZECH} "Nastavení instalace"

#LangString CP_HEADER_SUBTEXT ${LANG_ENGLISH} "Choose the settings for the installation"
LangString CP_HEADER_SUBTEXT ${LANG_CZECH} "Vyberte nastavení pro instalaci"

#LangString CP_TOP ${LANG_ENGLISH} "Check or uncheck the options for the installation.$\r$\nClick 'Next' to continue."
LangString CP_TOP ${LANG_CZECH} "Označ nebo zruš možnosti pro instalaci.$\r$\nKlikni na 'Další' pro pokračování."

#LangString CP_INSTTYPE ${LANG_ENGLISH} "Installation type:"
LangString CP_INSTTYPE ${LANG_CZECH} "Typ instalace:"

#LangString CP_COMPLIST ${LANG_ENGLISH} "Adjust the settings individually:"
LangString CP_COMPLIST ${LANG_CZECH} "Nastavit možnosti jednotlivě:"

#LangString Page_SetConfig_Title ${LANG_ENGLISH} "Default configuration settings"
LangString Page_SetConfig_Title ${LANG_CZECH} "Výchozí nastavení konfigurace"

#LangString Page_SetConfig_Subtitle ${LANG_ENGLISH} "Preconfigure settings for OpenCPN"
LangString Page_SetConfig_Subtitle ${LANG_CZECH} "Přednastavit možnosti pro OpenCPN"

#LangString Select_Chart_Directory ${LANG_ENGLISH} "Please select a chart directory to add to OpenCPN"
LangString Select_Chart_Directory ${LANG_CZECH} "Prosím vyberte složku map pro přidání do OpenCPN"

#LangString BT_Browse_SetConfig_Text ${LANG_ENGLISH} "Browse..."
LangString BT_Browse_SetConfig_Text ${LANG_CZECH} "Projít..."

#LangString BT_DeleteAll_SetConfig_Text ${LANG_ENGLISH} "Delete ALL"
LangString BT_DeleteAll_SetConfig_Text ${LANG_CZECH} "Smazat VŠE"

#LangString BT_Delete_SetConfig_Text ${LANG_ENGLISH} "Delete"
LangString BT_Delete_SetConfig_Text ${LANG_CZECH} "Smazat"

#LangString GB_ChartDir_SetConfig_Text ${LANG_ENGLISH} "Add Chart Directory"
LangString GB_ChartDir_SetConfig_Text ${LANG_CZECH} "Přidat složku map"

#LangString CB_Language_SetConfig_Text ${LANG_ENGLISH} "Set OpenCPN program language to $LANG_NAME"
LangString CB_Language_SetConfig_Text ${LANG_CZECH} "Nastavit jazyk OpenCPN na $LANG_NAME"

#LangString GB_Language_SetConfig_Text ${LANG_ENGLISH} "Set OpenCPN program language"
LangString GB_Language_SetConfig_Text ${LANG_CZECH} "Nastavit jazyk programu OpenCPN"

#LangString Label_SetConfig_Text ${LANG_ENGLISH} "Configure preconfig settings for the OpenCPN program.$\r$\nThese settings are saved in the file opencpn.ini."
LangString Label_SetConfig_Text ${LANG_CZECH} "Nastavit výchozí možnosti pro program OpenCPN.$\r$\nToto nastavení je uloženo v souboru opencpn.ini."

#LangString Page_Confirm_Title ${LANG_ENGLISH} "Ready to Install"
LangString Page_Confirm_Title ${LANG_CZECH} "Připraven k instalaci"

#LangString Page_Confirm_Subtitle ${LANG_ENGLISH} "Check the installation options"
LangString Page_Confirm_Subtitle ${LANG_CZECH} "Zkontrolovat možnosti instalace"

#LangString Installation_Type ${LANG_ENGLISH} "Setup type"
LangString Installation_Type ${LANG_CZECH} "Typ instalace"

#LangString Installation_Directory ${LANG_ENGLISH} "Destination location"
LangString Installation_Directory ${LANG_CZECH} "Cílové umístění"

#LangString Create_Shortcuts ${LANG_ENGLISH} "Create shortcuts"
LangString Create_Shortcuts ${LANG_CZECH} "Vytvořit zástupce"

#LangString Type_Reinstall ${LANG_ENGLISH} "Reinstallation"
LangString Type_Reinstall ${LANG_CZECH} "Přeinstalovat"

#LangString Type_Upgrade ${LANG_ENGLISH} "Upgrade"
LangString Type_Upgrade ${LANG_CZECH} "Upgrade"

#LangString Type_Downgrade ${LANG_ENGLISH} "Downgrade"
LangString Type_Downgrade ${LANG_CZECH} "Downgrade"

#LangString Type_Parallel_Installation ${LANG_ENGLISH} "Parallel installation"
LangString Type_Parallel_Installation ${LANG_CZECH} "Souběžná instalace"

#LangString Type_Fresh_Installation ${LANG_ENGLISH} "New installation"
LangString Type_Fresh_Installation ${LANG_CZECH} "Nová instalace"

#LangString Shortcuts ${LANG_ENGLISH} "Shortcuts"
LangString Shortcuts ${LANG_CZECH} "Zástupci"

#LangString Create_Shortcut_Start_Menu ${LANG_ENGLISH} "in the Start Menu folder"
LangString Create_Shortcut_Start_Menu ${LANG_CZECH} "v nabídce Start"

#LangString Create_Shortcut_Desktop ${LANG_ENGLISH} "on the Desktop"
LangString Create_Shortcut_Desktop ${LANG_CZECH} "na ploše"

#LangString not_selected ${LANG_ENGLISH} "not selected"
LangString not_selected ${LANG_CZECH} "není vybráno"

#LangString text_none ${LANG_ENGLISH} "none"
LangString text_none ${LANG_CZECH} "nic"

#LangString SecGrpResetConfig_Text ${LANG_ENGLISH} "Reset ALL OpenCPN configuration files"
LangString SecGrpResetConfig_Text ${LANG_CZECH} "Vymazat VŠECHNY konfigurační soubory OpenCPN"

#LangString SecDeleteConfigFile_Text ${LANG_ENGLISH} "Delete config file (opencpn.ini)"
LangString SecDeleteConfigFile_Text ${LANG_CZECH} "Smazat soubor nastavení (opencpn.ini)"

#LangString SecDeleteChartDB_Text ${LANG_ENGLISH} "Delete Chart Database file (CHRTLIST.DAT)"
LangString SecDeleteChartDB_Text ${LANG_CZECH} "Smazat soubor databáze map (CHRTLIST.DAT)"

#LangString SecDeleteSENC_Text ${LANG_ENGLISH} "Delete SENC directory"
LangString SecDeleteSENC_Text ${LANG_CZECH} "Smazat složku SENC"

#LangString SecDeleteCM93_Text ${LANG_ENGLISH} "Delete CM93 Directory"
LangString SecDeleteCM93_Text ${LANG_CZECH} "Smazat adresář CM93"

#LangString SecDeleteLogFile_Text ${LANG_ENGLISH} "Delete Log file (opencpn.log)"
LangString SecDeleteLogFile_Text ${LANG_CZECH} "Smazat soubor logu (opencpn.log)"

#LangString SecDeleteNavobjFile_Text ${LANG_ENGLISH} "Delete Navobject file (navobj.xml)"
LangString SecDeleteNavobjFile_Text ${LANG_CZECH} "Smazat soubor Navobject (navobj.xml)"

#LangString SecSetConfig_Text ${LANG_ENGLISH} "OpenCPN Configuration Settings"
LangString SecSetConfig_Text ${LANG_CZECH} "Možnosti nastavení OpenCPN"

#LangString SecSetConfigLang_Text ${LANG_ENGLISH} "Set language to $LANG_NAME"
LangString SecSetConfigLang_Text ${LANG_CZECH} "Nastavit jazyk na $LANG_NAME"

#LangString SecSetConfigChartDir_Text ${LANG_ENGLISH} "Select Chart Directories"
LangString SecSetConfigChartDir_Text ${LANG_CZECH} "Vybrat složky map"

#LangString SecGrpResetConfig_Desc ${LANG_ENGLISH} "Deletes ALL files and subdirectories in the configuration folder."
LangString SecGrpResetConfig_Desc ${LANG_CZECH} "Smaže VŠECHNY soubory a podsložky ve složce konfigurace."

#LangString SecDeleteConfigFile_Desc ${LANG_ENGLISH} "Deletes the configuration file. This file helds also Routes and Waypoints."
LangString SecDeleteConfigFile_Desc ${LANG_CZECH} "Smaže konfigurační soubor. Tento soubor obsahuje také trasy a trasové body."

#LangString SecDeleteChartDB_Desc ${LANG_ENGLISH} "Deletes the Chart DB file."
LangString SecDeleteChartDB_Desc ${LANG_CZECH} "Smaže soubor DB map."

#LangString SecDeleteSENC_Desc ${LANG_ENGLISH} "Deletes the SENC subfolder which helds the Bitmaps and S57 chart cache files."
LangString SecDeleteSENC_Desc ${LANG_CZECH} "Smaže podsložku SENC, která obsahuje bitmapy a cache S57 map."

#LangString SecDeleteCM93_Desc ${LANG_ENGLISH} "Deletes the cm93 subfolder which helds the cached cm93 chart outlines."
LangString SecDeleteCM93_Desc ${LANG_CZECH} "Smaže podsložku cm93, která obsahuje cache obrysů map CM93."

#LangString SecDeleteNavobjFile_Desc ${LANG_ENGLISH} "Deletes the file 'navobj.xml' in the config folder. This file contains the Tracks."
LangString SecDeleteNavobjFile_Desc ${LANG_CZECH} "Smaže soubor 'navobj.xml' ve složce nastavení. Tento soubor obsahuje stopy."

#LangString SecDeleteLogFile_Desc ${LANG_ENGLISH} "Deletes the OpenCPN log file 'opencpn.log'."
LangString SecDeleteLogFile_Desc ${LANG_CZECH} "Smaže soubor logu OpenCPN 'opencpn.log'."

#LangString SecGrpShortcuts_Desc ${LANG_ENGLISH} "Creates shortcuts in both, the Start Menu and on the Desktop."
LangString SecGrpShortcuts_Desc ${LANG_CZECH} "Vytvoří zástupce na ploše i v nabídce Start."

#LangString SecShortcutStartmenu_Desc ${LANG_ENGLISH} "Creates shortcuts for OpenCPN and the Uninstaller in the Start Menu - configurable on the next page."
LangString SecShortcutStartmenu_Desc ${LANG_CZECH} "Vytvoří zástupce pro OpenCPN a deinstalátor v nabádce Start - nastavitelné na další stránce."

#LangString SecShortcutDesktop_Desc ${LANG_ENGLISH} "Creates a shortcut for OpenCPN on the Desktop."
LangString SecShortcutDesktop_Desc ${LANG_CZECH} "Vytvoří zástupce pro OpenCPN na ploše."

#LangString SecSetConfig_Desc ${LANG_ENGLISH} "Preconfigure the language setting and the chart dir(s) for OpenCPN. When selected a separate page is shown for configuration."
LangString SecSetConfig_Desc ${LANG_CZECH} "Přednastavit jazyk a složky map pro OpenCPN. Pokud je vybráno, je zobrazena zvláštní stránka pro nastavení."

#LangString UnSecDeleteConfigFolder_Desc ${LANG_ENGLISH} "Deletes the config folder and all its files and subdirectories."
LangString UnSecDeleteConfigFolder_Desc ${LANG_CZECH} "Smaže složku s nastavením a všechny její soubory a podsložky."

#LangString Uninstall_Link ${LANG_ENGLISH} "Uninstall OpenCPN 4.1.718"
LangString Uninstall_Link ${LANG_CZECH} "Odinstalovat OpenCPN 4.1.718"

#LangString Page_TypeInst_Title ${LANG_ENGLISH} "Previous Installations"
LangString Page_TypeInst_Title ${LANG_CZECH} "Předchozí instalace"

#LangString Page_TypeInst_Subtitle ${LANG_ENGLISH} "$MULTIPLE_INSTALLS installations of OpenCPN found.$\r$\nChoose how you want to install OpenCPN."
LangString Page_TypeInst_Subtitle ${LANG_CZECH} "$MULTIPLE_INSTALLS instalace OpenCPN nalezeny.$\r$\nVyberte, jak chcete OpenCPN nainstalovat."

#LangString Page_TypeInst_Title_1 ${LANG_ENGLISH} "Already Installed"
LangString Page_TypeInst_Title_1 ${LANG_CZECH} "Již nainstalováno"

#LangString Page_TypeInst_Subtitle_1 ${LANG_ENGLISH} "Found installed version of OpenCPN.$\r$\nChoose how you want to install OpenCPN."
LangString Page_TypeInst_Subtitle_1 ${LANG_CZECH} "Nalezena nainstalovaná verze OpenCPN.$\r$\nVyberte, jak chcete OpenCPN nainstalovat."

#LangString Page_TypeInst_Title_Exists ${LANG_ENGLISH} "Version Already Installed"
LangString Page_TypeInst_Title_Exists ${LANG_CZECH} "Verze již nainstalována"

#LangString Page_TypeInst_Subtitle_Exists ${LANG_ENGLISH} "OpenCPN 4.1.718 is already installed.$\r$\nChoose how you want to install OpenCPN."
LangString Page_TypeInst_Subtitle_Exists ${LANG_CZECH} "OpenCPN 4.1.718 je již nainstalováno.$\r$\nVyberte, jak chcete OpenCPN nainstalovat."

#LangString Label_TypeInst_Reinstall_Text ${LANG_ENGLISH} "OpenCPN 4.1.718 is already installed on your system. You can reinstall the version now using the previous settings. If you wish to install the program with different settings, cancel installation and un-install the existing version first. Click Next to continue."
LangString Label_TypeInst_Reinstall_Text ${LANG_CZECH} "OpenCPN 4.1.718 je již na Vašem systému nainstalováno. Nyní můžete přeinstalovat verzi s původní nastavením. Pokud chcete nainstalovat program s jiným nastavením, zrušte instalaci a odinstalujte napřed původní verzi. Klikněte na Další pro pokračování."

#LangString RB_Reinstall_TypeInst_Text ${LANG_ENGLISH} "Reinstall using previous settings"
LangString RB_Reinstall_TypeInst_Text ${LANG_CZECH} "Přeinstalovat s původním nastavením"

#LangString Label_TypeInst_Upgrade_Text ${LANG_ENGLISH} "An older version of OpenCPN is installed on your system. Select the installation type you want to perform and click Next to continue."
LangString Label_TypeInst_Upgrade_Text ${LANG_CZECH} "Starší verze OpenCPN je nainstalována na Vašem systému. Zvolte typ instalace a klikněte na Další pro pokračování."

#LangString RB_Upgrade_TypeInst_Text ${LANG_ENGLISH} "Upgrade OpenCPN using previous settings (recommended)"
LangString RB_Upgrade_TypeInst_Text ${LANG_CZECH} "Povýšit OpenCPN s původním nastavením (doporučeno)"

#LangString Label_TypeInst_Downgrade_Text ${LANG_ENGLISH} "A newer version of OpenCPN is installed on your system. It is not recommended to downgrade to an older version. Select the installation type you want to perform and click Next to continue."
LangString Label_TypeInst_Downgrade_Text ${LANG_CZECH} "Novější verze OpenCPN je nainstalována na Vašem systému. Není doporučováno downgradovat na starší verzi. Zvolte typ instalace, který chcete provést a klikněte na Další pro pokračování."

#LangString RB_Downgrade_TypeInst_Text ${LANG_ENGLISH} "Downgrade OpenCPN using previous settings (recommended)"
LangString RB_Downgrade_TypeInst_Text ${LANG_CZECH} "Downgrade OpenCPN s původním nastavením (doporučeno)"

#LangString Label_TypeInst_Parallel_Text ${LANG_ENGLISH} "Setup has detected several installed versions of OpenCPN on your system. It is recommended to have only one version of OpenCPN installed at a time. Select the operation you want to perform and click Next to continue."
LangString Label_TypeInst_Parallel_Text ${LANG_CZECH} "Instalátor detekoval několik instalovaných verzí OpenCPN na Vašem systému. Je doporučeno mít instalovánu pouze jednu verzi OpenCPN. Vyberte operaci, kterou chcete provést a klikněte na Další pro pokračování."

#LangString RB_Parallel_TypeInst_Text ${LANG_ENGLISH} "Parallel Installation (advanced users)"
LangString RB_Parallel_TypeInst_Text ${LANG_CZECH} "Paralelní instalace (pokročilí uživatelé)"

#LangString EmptyString ${LANG_ENGLISH} ""
LangString EmptyString ${LANG_CZECH} ""

#LangString Show_Readme_Text ${LANG_ENGLISH} "Show Install Log file"
LangString Show_Readme_Text ${LANG_CZECH} "Zobraz log instalace"

#LangString Finish_Link_Text ${LANG_ENGLISH} "OpenCPN website (Internet connection required)"
LangString Finish_Link_Text ${LANG_CZECH} "Web OpenCPN (Je potřeba připojení k Internetu)"

#LangString Finish_Link_URL ${LANG_ENGLISH} "http://opencpn.info/?ref=installer&user=$USERS_NAME&lang=$REG_LANG&version=4.1.718"
LangString Finish_Link_URL ${LANG_CZECH} "http://opencpn.info/?ref=installer&user=$USERS_NAME&lang=$REG_LANG&version=4.1.718"

#LangString Msg_UAC_Admin_Retry ${LANG_ENGLISH} "This $Inst_Or_Uninst requires admin access!$\r$\nPlease log on as an Administrator and try again."
LangString Msg_UAC_Admin_Retry ${LANG_CZECH} "Tento $Inst_Or_Uninst vyžaduje přístup administrátora!$\r$\nProsím, přihlaste se jako Administrator a zkuste to znovu."

#LangString Msg_UAC_Admin_Abort ${LANG_ENGLISH} "This $Inst_Or_Uninst requires admin privileges, aborting!"
LangString Msg_UAC_Admin_Abort ${LANG_CZECH} "Tento $Inst_Or_Uninst vyžaduje administrátorská oprávnění, končím!"

#LangString Msg_UAC_Logon_Abort ${LANG_ENGLISH} "Logon service not running, aborting!"
LangString Msg_UAC_Logon_Abort ${LANG_CZECH} "Logon služba neběží, končím!"

#LangString Msg_UAC_No_Elevation ${LANG_ENGLISH} "Unable to elevate , error $0"
LangString Msg_UAC_No_Elevation ${LANG_CZECH} "Nemohu povýšit , chyba $0"

#LangString Msg_Error_Remove_InstDir ${LANG_ENGLISH} "The installation directory$\r$\n'$INSTDIR'$\r$\ncould not be removed because it is not empty."
LangString Msg_Error_Remove_InstDir ${LANG_CZECH} "Instalační složka$\r$\n'$INSTDIR'$\r$\nnelze odstranit, protože není prázdná."

#LangString Msg_Path_Invalid ${LANG_ENGLISH} "The entered path is invalid!$\r$\n$\r$\nPlease check your input."
LangString Msg_Path_Invalid ${LANG_CZECH} "Zadaná cesta je neplatná!$\r$\n$\r$\nZkontrolujte prosím své zadání."

#LangString Msg_Path_Installation_Exists ${LANG_ENGLISH} "There already exists an installation of OpenCPN in the given path.$\r$\nIf you wish to use this path, first uninstall the version. Or select another path for this installation."
LangString Msg_Path_Installation_Exists ${LANG_CZECH} "Instalace OpenCPN v zadané cestě již existuje.$\r$\nPokud chcete použít tuto cestu, nejprve odinstalujte tuto verzi. Nebo pro tuto instalaci zvolte jinou cestu."

#LangString Msg_Uninstall_Old_Versions ${LANG_ENGLISH} "Setup has detected at least one older version of OpenCPN which is not compatible with the new version control.$\r$\nIt is recommended to first uninstall all of these installations (this does not affect your Config files) and clean-up the Registry.$\r$\nIf you click Yes on the next page a listbox with all installation locations will be shown and you will have the possibility to select the installations you wish to uninstall.$\r$\nIf you are unsure about possible other previous installations than those shown in the listbox, you can also perform a scan of the entire Harddisk (ATTENTION: This may take several minutes) to find all install locations (recommended).$\r$\n$\r$\nDo you want to uninstall the old version(s)?"
LangString Msg_Uninstall_Old_Versions ${LANG_CZECH} "Instalátor detekoval alespoň jednu starší verzi OpenCPN, která není komúatibilní s novou správou verzí.$\r$\nJe doporučeno nejprve odstranit všechny tyto instalace (neovlivní to Vaše konfigurační soubory) a vyčistit Registr.$\r$\nPokud na dalši stránce kliknete na Ano, bude zobrazen seznam všech umístění instalace a budete mít možnost vybrat instalace, které chcete odebrat.$\r$\nPokud si nejste jist dalšími možnými instalacemi kromě těch na seznamu, můžete provést sken celého pevného disku (POZOR: může to trvat několik minut), aby byly nalezeny všechny umístění instalace (doporučeno).$\r$\n$\r$\nChcete odinstalovat předchozí verze?"

#LangString Msg_Not_All_Old_Uninstalled ${LANG_ENGLISH} "You have not uninstalled all previous installations.$\r$\n$\r$\nDo you want to continue anyway?"
LangString Msg_Not_All_Old_Uninstalled ${LANG_CZECH} "Neodinstaloval jste všechny předchozí instalace.$\r$\n$\r$\nChcete přesto pokračovat?"

#LangString Msg_Uninstaller_Param_Help ${LANG_ENGLISH} "Uninstaller Help$\r$\n$\r$\nUse the command line or a shortcut to run the uninstaller:$\r$\n$INSTDIR\Uninstall 4.1.718.exe [/?] | [/S], [/type=X]$\r$\n$\r$\nParameters:$\r$\n/?$\tshows this help message box$\r$\n/S$\truns the uninstaller in silent mode$\r$\n$\twill do a full uninstall if called without any type parameter$\r$\n/type=X$\tX must be a digit between 1 and 5$\r$\n$\t1 = Removes only the program files$\r$\n$\t2 = Removes only the program files + preconfig RegKey$\r$\n$\t3 = Removes the program files + config folder$\r$\n$\tpreserves the preconfig RegKey$\r$\n$\t4 = Deletes only the config folder$\r$\n$\t5 = Deletes only the preconfig RegKey$\r$\n$\r$\nEntering any other number or value will quit the uninstaller with a message box without removing or deleting anything."
LangString Msg_Uninstaller_Param_Help ${LANG_CZECH} "Nápověda deinstalátoru$\r$\n$\r$\nPoužijte příkazovou řádku nebo zástupce ke spuštění odinstalátoru:$\r$\n$INSTDIR\Uninstall 4.1.718.exe [/?] | [/S], [/type=X]$\r$\n$\r$\nParametrys:$\r$\n/?$\tzobrazí tuto nápovědu$\r$\n/S$\tspustí deinstalátor v tichém módu$\r$\n$\tprovede kompletní odinstalaci, pokud je spuštěn bez parametrů$\r$\n/type=X$\tX musí být číslo mezi 1 a 5$\r$\n$\t1 = Odstraní jen programové soubory$\r$\n$\t2 = Odstraní jen programové soibory + přednastavení RegKey$\r$\n$\t3 = Odstraní programové soubory + složku nastavení$\r$\n$\tzachová přednastavení RegKey$\r$\n$\t4 = Smaže jen složku nastavení$\r$\n$\t5 = Smaže jen přednastavení RegKey$\r$\n$\r$\nVložení jakéhokoli jiného čísla ukončí odinstalátor se zprávou bez toho, aby cokoli odstranil nebo smazal."

#LangString Msg_Unknown_Type_Param ${LANG_ENGLISH} "The entered value '$9' for the type parameter is not valid!$\r$\nUse $INSTDIR\Uninstall 4.1.718.exe /?$\r$\nto see a list of valid values."
LangString Msg_Unknown_Type_Param ${LANG_CZECH} "Vložená hodnota '$9' pro parametr typu není platná!$\r$\nPoužijte $INSTDIR\Uninstall 4.1.718.exe /?$\r$\npro zobrazení seznamu povolených hodnot."

;added on 2010-10-16
#LangString SecGrpShortcuts_Text ${LANG_ENGLISH} "Create Shortcuts"
LangString SecGrpShortcuts_Text ${LANG_CZECH} "Vytvořit zástupce"

#LangString SecShortcutStartmenu_Text ${LANG_ENGLISH} "Add Shortcuts to the Start Menu"
LangString SecShortcutStartmenu_Text ${LANG_CZECH} "Vytvořit zástupce v nabídce Start"

#LangString SecShortcutDesktop_Text ${LANG_ENGLISH} "Create Shortcut on the Desktop"
LangString SecShortcutDesktop_Text ${LANG_CZECH} "Vytvořit zástupce na ploše"

#LangString Installation_Delete_Config ${LANG_ENGLISH} "Delete (existing) Config Subdirectories/ Files"
LangString Installation_Delete_Config ${LANG_CZECH} "Smazat (existující) konfigurační podsložky/ soubory"

;added on 2010-10-17
#LangString Banner_Scan_Drive ${LANG_ENGLISH} "Scanning Drive $9"
LangString Banner_Scan_Drive ${LANG_CZECH} "Skenuji disk $9"

#LangString Banner_Installation_Found_Singular ${LANG_ENGLISH} "Found $counter_tmp installation"
LangString Banner_Installation_Found_Singular ${LANG_CZECH} "Nalezena $counter_tmp instalace"

#LangString Banner_Installation_Found_Plural ${LANG_ENGLISH} "Found $counter_tmp installations"
LangString Banner_Installation_Found_Plural ${LANG_CZECH} "Nalezeny $counter_tmp instalace"

;added on 2010-10-18
#LangString UnSecDeleteConfigFolder_Text ${LANG_ENGLISH} "Delete Config folder"
LangString UnSecDeleteConfigFolder_Text ${LANG_CZECH} "Smazat složku konfigurace"

;added on 2010-10-21
#LangString Uninstall_Folder_Name ${LANG_ENGLISH} "Uninstall"
LangString Uninstall_Folder_Name ${LANG_CZECH} "Odinstalovat"

;added on 2011-06-13
#LangString ^NameDA ${LANG_ENGLISH} "OpenCPN Version 4.1.718"
LangString ^NameDA ${LANG_CZECH} "OpenCPN Verze 4.1.718"

;added on 2011-07-24
#LangString License_Message ${LANG_ENGLISH} "OpenCPN is software libre. You are encouraged and legally entitled to copy, reinstall, modify, and redistribute this program for yourself and your friends under the terms of the GPL. $\r$\nSmooth sailing!"
LangString License_Message ${LANG_CZECH} "OpenCPN je svobodný software. Můžete a budeme rádi, když ho budete kopírovat, instalovat, modifikovat a šířit ať už pro sebe nebo své přátele, pokud dodržíte podmínky licence GPL. $\r$\nŠťastnou plavbu!"

#LangString Installer_Unsupported_XP_SP_Version ${LANG_ENGLISH} "Your Windows XP Service Pack version is not supported (it's too old)!$\r$\nYou need at least Windows XP SP3."
LangString Installer_Unsupported_XP_SP_Version ${LANG_CZECH} "Verze instalovaného service packu Windows XP není podporována (je prříliš stará)!$\r$\nPotřebuješ alespoŇ Windows XP SP3."
